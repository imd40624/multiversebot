...
async def on_reaction_add(reaction, user):
  if reaction.message.server is None:
      ...
      if reaction.emoji == '🏵':
        ...
        embed.add_field(name = 'Setting up Welcomer log(Admin Permission required) ',value ='Use mv!setupwelcomer. It will add a welcome channel. Just put that channel in your desired category and it will send all logs there.\n\n**__Note:__** **There is an update from which you can setup channel by just tagging that channel.** __Ex:__ ``mv!set-welcome-channel #channel``. **If you wanna remove that channel from setting then use** ``mv!unset-welcome-channel`` **and if you wanna get the welcome channel name then use** ``mv!get-welcome-channel``. Also you can set welcome message similar to this. i.e ``mv!set-welcome-message <message>`` but for that you must need to activate it by using ``mv!set-welcome-channel #channel`` to make it work.',inline = False)
